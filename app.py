...
def get_scheduled_jobs():
    try:
        ...
        jobs = []
        for job in email_jobs_collection.find():
            jobs.append({
                'id': str(job['_id']),
                'subject': job.get('subject', ''),
                'email_count': job.get('email_count', 0),
...
def cancel_job(job_id):
    try:
        ...
        result = email_jobs_collection.update_one(
            {'_id': ObjectId(job_id), 'status': 'scheduled'},
            {'$set': {'status': 'cancelled'}}
        )

        if result.modified_count == 0:
            return jsonify({'success': False, 'error': 'Job not found or already processed'})

        # Remove job from scheduler if still scheduled
        try:
            scheduler.remove_job(job_id)
        except Exception as e:
            logger.warning(f"Job {job_id} may not exist in scheduler: {e}")
